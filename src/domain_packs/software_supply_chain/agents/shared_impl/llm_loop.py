...
def _build_decision_system_prompt(normalized_input: _NormalizedSupplyChainInput) -> str:
...
def _build_loop_payload(
    ...
    *,
    ...
    return {
        ...
        "github_context": _repository_context_payload(normalized_input),
...
def _normalize_decision_output(
    ...
    *,
    ...
    if missing:
        return None, "SSC_DECISION_INCOMPLETE", "Structured decision is missing required keys: " + ", ".join(missing)
    decision_type = str(llm_output.get("decision_type", "")).strip().lower()
    if decision_type not in {"respond", "tool_call"}:
        return None, "SSC_DECISION_INVALID", "decision_type must be either 'respond' or 'tool_call'"
...
def _build_final_response_system_prompt(normalized_input: _NormalizedSupplyChainInput) -> str:
    return "\n\n".join(
        [
            normalized_input.system_prompt,
    ...
    *,
    ...
    content = "\n".join(
        [
            ...
            (
                f"Active repository target: {normalized_input.current_repo_url}"
            ...
            memory_id = services.memory_provider.write(
                {
                    ...
                    "tags": [
                        ...
                        "coding_task",
                        "edit" if payload["edited"] else "read_only",
                        loop_stop_reason,
                    ],
