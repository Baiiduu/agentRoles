...
from .shared import (
    ...
    _artifact_type,
    _emit_progress,
    _is_skill_inventory_request,
    _normalize_input,
    _primary_memory_scope,
    _skill_inventory_reply,
)
from .state import (
...
class SoftwareSupplyChainChatImplementation:
    implementation_ref = "software_supply_chain.base"
    ...
    def invoke(self, context: ExecutionContext) -> NodeExecutionResult:
        ...
        if not normalized_input.message:
            return NodeExecutionResult(
                ...
                error_code="SSC_MESSAGE_REQUIRED",
                error_message="message must be non-empty",
            )
        if _is_skill_inventory_request(normalized_input.message):
            reply = _skill_inventory_reply(normalized_input)
            return NodeExecutionResult(
        ...
        if services is None or services.llm_invoker is None:
            fallback = "Software supply chain agent is ready, but no LLM provider is configured yet."
            return NodeExecutionResult(
                status=NodeStatus.SUCCEEDED,
                output={
                    "reply": fallback,
        ...
        for step in range(1, max_steps + 1):
            ...
            if decision is None:
                return NodeExecutionResult(
                    status=NodeStatus.FAILED,
                    error_code=error_code or "SSC_DECISION_FAILED",
        ...
        if final_reply is None:
            ...
            if final_reply is None:
                return NodeExecutionResult(
                    status=NodeStatus.FAILED,
                    error_code=str(final_llm_context.get("error_code", "SSC_FINALIZE_FAILED")),
...
def build_software_supply_chain_implementations(
    implementation_types: list[type[AgentImplementation]],
) -> list[AgentImplementation]:
    return [implementation_type() for implementation_type in implementation_types]
