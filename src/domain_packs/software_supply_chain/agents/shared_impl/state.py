...
def _build_working_summary(
    *,
    ...
    if normalized_input.current_repo_url:
        known_facts.append("Active repository target: " + normalized_input.current_repo_url)
    if normalized_input.saved_repo_urls:
        known_facts.append(
            "Saved repository candidates: "
            + ", ".join(normalized_input.saved_repo_urls[:3])
        )
    pending_by_phase = {
...
def _task_memory_snapshot(
    *,
    ...
    if normalized_input.current_repo_url:
        confirmed_facts.append("Active repository target: " + normalized_input.current_repo_url)
    ...
    if not pending_next_step:
        pending_next_step = "Run the next software supply chain step based on the latest repository evidence."
    return {
